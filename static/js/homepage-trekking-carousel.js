document.addEventListener("DOMContentLoaded", function () {
  const shell = document.querySelector("[data-trekking-carousel]");
  const track = document.getElementById("trekkingToursTrack");
  if (!shell || !track) return;

  const cards = Array.from(track.querySelectorAll(".dynamic-trek-card"));
  const controls = Array.from(
    document.querySelectorAll('[data-trekking-scroll="trekkingToursTrack"]')
  );
  const delay = Math.max(
    2500,
    Number(shell.getAttribute("data-autoplay-delay")) || 4500
  );
